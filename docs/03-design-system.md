...
- **Bottom tab bar:** fixed, 60px tall plus the safe area, four tabs: **Program | Honorees | Tributes | About** (D32). Each tab is its label’s width plus an equal share of the remaining space, so the gaps between labels are equal (Blake, 2026-09-26); the whole bar stays tappable. The active tab turns gold; Tributes covers the ads and the closing, and About covers the footer. Labels must fit ~78px at 15px bold (320px screens). A few lines of the page’s ~1.5 KB of JavaScript track the visible section. Without JS, links still work.
...
- Tab bar: one 3px gold marker glides between tabs (320ms), matching the active tab’s position and width. The page script measures the active tab (on change, resize, and font load) into `--tab-x`/`--tab-w`; `:has()` shows it. With reduced motion, the per-tab bar returns.
